...
# myModel class
class myModel():
    def __init__(self, nclasses, optmizer_index=0, CNN_filters=10, 
        ...
        self.__y_train = to_categorical(y_test, nclasses)
...
def run_model(nclasses, optmizer_index=0, CNN_filters=10, 
              CNN_filter_size=5, CNN_MaxPool_size=2, CNN_layers=2, CNN_activation_index=0, DNN_neurons=40, 
              DNN_layers=2, DNN_activation_index=0, dropout=0.2, batch_size=100, epochs=50):
    
    _model = myModel(nclasses, optmizer_index, CNN_filters, CNN_MaxPool_size, CNN_filter_size,
...
def f(x):
    print(x)
    evaluation = run_model(nclasses=10,
